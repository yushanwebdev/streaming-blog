...
const readFileOr404 = (...args) =>
  readFile(...args).catch((err) => {
...
router.use(
  "/static-rev",
  express.static(__dirname + "/../static-rev", { maxAge: "1y" })
);
router.use(
  "/favicon.ico",
  express.static(__dirname + "/../static/favicon.ico", { maxAge: "1y" })
);
      ...
      await Promise.all(
        slugs.map(async (slug) => {
          ...
          const meta = JSON.parse(
            await readFile(`${itemDir}/meta.json`, "utf-8")
          );
